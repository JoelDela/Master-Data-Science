...
datos<-read.csv("./data_in/datos_laliga.csv")
...
aggregate(average ~ posicion, data = datos, FUN = mean)

aggregate(average ~ posicion, data = datos, FUN = sd)
...
ggplot(data = datos, aes(x = posicion, y = average, color = posicion)) +
  geom_boxplot() +
  theme_bw()
...
by(data = datos,iNDiCES = datos$posicion,FUN = function(x){ lillie.test(x$average)})
...
fligner.test(average ~ factor(posicion),datos)


leveneTest(average ~ factor(posicion),datos,center = "median")
...
anova <- aov(datos$average ~ datos$posicion)
summary(anova)
