...
data_scrap[grep("Arroyomol",data_scrap$level5),]
...
t.test(df_temp$price,alternative = "two.sided",mu=295000)
...
sapply(data_scrap[,c("price","rooms","surface","bathrooms")],function(x){round(sd(x,na.rm=T),2)})
...
aggregate(data_scrap[,c("price","rooms","surface","bathrooms")], by = list(localidad=data_scrap$level5), FUN = function(x){round(sd(x,na.rm=T),2)})
...
#Primero eliges una paleta de colores que discriminará según 'level5'
...
#Después pintamos el mapa y añadimos unos markers en las longitudes y latitudes que indiques, el popup indica lo que
#se va a enseñar en ese punto, hemos elegido el id. Pal es la paleta de colores, discriminará según level5
...
cor(data_scrap$price,data_scrap$surface)


#soluciona el problema (si lo hay)

data_scrap<-data_scrap[which(!is.na(data_scrap$price) & !is.na(data_scrap$surface)),]
...
cor.test(data_scrap$price,data_scrap$surface)
...
data_scrap$zipCode<-NULL #este NULL debe ir aqui
data_scrap$customZone<-NULL #este NULL debe ir aqui
...
anova <- aov(pricem2~surface,data=data_scrap)
summary(anova)
plot(anova)
...
aggregate(price~level5,data=data_scrap,function(x){mean(x,na.rm=T)})

ansari.test(price ~level5,data=data_scrap)
